...
public class uint232 implements uint {
    ...
    public uint232(BigInteger value) {
        if (value.signum() < 0 || value.compareTo(MAX_VALUE) > 0) {
            throw new IllegalArgumentException("Value is out of range for uint232");
        }
        this.value = value;
    }
    ...
    public uint _add(uint other) {
        return new uint232(this.value().add(other.value()));
    }

    @Override
    public uint _sub(uint other) {
        return new uint232(this.value().subtract(other.value()));
    }

    @Override
    public uint _mul(uint other) {
        return new uint232(this.value().multiply(other.value()));
    }

    @Override
    public uint _div(uint other) {
        return new uint232(this.value().divide(other.value()));
    }

    @Override
    public uint _mod(uint other) {
        return new uint232(this.value().mod(other.value()));
    }

    @Override
    public uint _pow(uint exponent) {
        return new uint232(this.value().pow(exponent.value().intValue()));
    }
    ...
    public uint and(uint other) {
        return new uint232(this.value().and(other.value()));
    }

    @Override
    public uint or(uint other) {
        return new uint232(this.value().or(other.value()));
    }

    @Override
    public uint xor(uint other) {
        return new uint232(this.value().xor(other.value()));
    }

    @Override
    public uint not() {
        return new uint232(this.value().not());
    }

    @Override
    public uint shiftLeft(int n) {
        return new uint232(this.value().shiftLeft(n));
    }

    @Override
    public uint shiftRight(int n) {
        return new uint232(this.value().shiftRight(n));
    }
}
