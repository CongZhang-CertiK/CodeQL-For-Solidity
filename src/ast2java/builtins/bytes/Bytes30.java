...
public class Bytes30 implements IBytes {
    ...
    public Bytes30(int value) {
        for (int i = 0; i < SIZE; i++) {
            bytes[i] = (byte) ((value >> (8 * i)) & 0xFF);
        }
    }

    public Bytes30(String value) {
        BigInteger bigInt = new BigInteger(value, 16);
        byte[] temp = bigInt.toByteArray();
        int len = temp.length;
        if (len > SIZE) {
            throw new IllegalArgumentException("Input string is too long to fit into Bytes30");
        }
